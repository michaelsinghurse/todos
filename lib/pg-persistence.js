...
module.exports = class PgPersistence {
  ...
  // combine to identify a legimate application user, `false` if either the 
  ...
  // list. Returns a promoise that resolves to `true` on success, `false` on 
  // failure.
  ...
  // resolves to `true` on success, `false` if the todo list isn't found.
  async setTodoListTitle(todoListId, title) {
    const TODO_TITLE = "UPDATE todolists" +
                       " SET title = $1" + 
                       " WHERE id = $2 AND username = $3";
    
    let result = await dbQuery(TODO_TITLE, title, todoListId, this.username);
    return result.rowCount > 0;
  }
